...
/// @brief 消息队列基类
/// MessageCallBackFunctionInfo消息回调函数信息类保存信息
...
void MessageCallBackFunctionFilterObj<T>::RegisterFunctionWithObj(
    int msgId, HandleFunctionWithObj function) {
...
bool MessageCallBackFunctionFilterObj<T>::IsFollowMsgId(Packet *packet) {
  // 先判断是否有无过滤的handle
  if (MessageCallBackFunction::IsFollowMsgId(packet)) return true;
