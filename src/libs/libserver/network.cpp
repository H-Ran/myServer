...
{
    auto pConnectObj = DynamicObjectPool<ConnectObj>::GetInstance()->MallocObject(this, socket);
...
void Network::Select()
{
    ...
    const int nfds =
        ::select(fdmax + 1, &readfds, &writefds, &exceptfds, &timeout);
    if (nfds <= 0)
        return;
