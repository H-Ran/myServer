...
bool RobotMgr::Init() {
  if (!NetworkConnector::Init()) return false;
...
void RobotMgr::Update() {
  ...
  if (_nextShowInfoTime > Global::GetInstance()->TimeTick) return;

  _nextShowInfoTime = timeutil::AddSeconds(Global::GetInstance()->TimeTick, 10);
  ShowInfo();
}
...
void RobotMgr::NofityServer(RobotStateType iType) {
  if (_robots.size() != GlobalRobots::GetInstance()->GetRobotsCount()) return;

  auto iter = std::find_if(_robots.begin(), _robots.end(), [&iType](auto pair) {
    if (pair.second < iType) return true;
